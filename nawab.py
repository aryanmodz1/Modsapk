...
ADMIN_IDS = [5408005513]
BOT_TOKEN = "7633892545:AAGjlgEuStUZiWYpHejPlFiR77MkdHsafF8"
thread_count = 900
packet_size = 9
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        ...
        bot.send_message(chat_id, 
            f"馃殌 饾棓饾榿饾榿饾棶饾棸饾椄 饾棪饾榿饾棶饾椏饾榿饾棽饾棻馃敟\n\n"
            f"饾棫饾棶饾椏饾棿饾棽饾榿: {target_ip}:{target_port}\n"
            f"饾棓饾榿饾榿饾棶饾棸饾椄 饾棫饾椂饾椇饾棽: {duration} 饾悞饾悶饾悳.\n"
            f"饾棫饾椀饾椏饾棽饾棶饾棻饾榾: {thread_count}\n"
            f"饾棧饾棶饾棸饾椄饾棽饾榿 饾棪饾椂饾槆饾棽: {packet_size}\n"
            f"釟� 釟� @Nawab79 釟� 釟�")
        ...
        bot.send_message(chat_id, 
            f"饾棓饾榿饾榿饾棶饾棸饾椄 饾棛饾椂饾椈饾椂饾榾饾椀饾棽饾棻 饾棪饾槀饾棸饾棸饾棽饾榾饾榾饾棾饾槀饾椆饾椆饾槅 馃殌")
...
def attack_command(message):
    ...
    if in_cooldown:
        ...
        bot.send_message(
            chat_id,
            f"*鈴� Cooldown in progress! Please wait {minutes}m {de}s before starting another attack.*",
            parse_mode='Markdown'
        )
        return
    ...
    if is_admin(user_id):
        try:
            bot.send_message(chat_id, "*饾悘饾惀饾悶饾悮饾惉饾悶 饾悘饾惈饾惃饾惎饾悽饾悵饾悶 鉁�:\n<饾悎饾悘> <饾悘饾悗饾悜饾悡> <饾悡饾悎饾悓饾悇>.*", parse_mode='Markdown')
...
def process_attack_command(message, chat_id):
    try:
        ...
        if in_cooldown:
            ...
            bot.send_message(
                chat_id,
                f"*鈴� Cooldown in progress! Please wait {minutes}m {seconds}s before starting another attack.*",
                parse_mode='Markdown'
            )
            return
        ...
        if len(args) != 3:
            bot.send_message(chat_id, "*啶椸げ啶� 啶灌啶� 啶灌啷� 啶熰啶班ぞ啶� 啶呧啷囙え 馃樇*", parse_mode='Markdown')
            return
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By 釟� @Nawab79 釟�"  
    ...
    try:
        ...
        if save_admin_data(admin_data):
            ...
            try:
                bot.send_message(
                    int(new_admin_id),
                    "*馃帀 Congratulations! You have been promoted to admin!*\n"
...
def send_welcome(message):
    ...
    my_account_button = KeyboardButton("饾悓饾惒 饾悁饾悳饾悳饾惃饾惍饾惂饾惌馃彟")
    attack_button = KeyboardButton("馃殌 饾悁饾惌饾惌饾悮饾悳饾悿")
    markup.add(my_account_button, attack_button)

    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By 釟� @Nawab79 釟淺n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By 釟� @Nawab79 釟淺n\n"
...
def genkey_command(message):
    ...
    if len(cmd_parts) != 3:
        bot.send_message(chat_id, (
            ...
            "- hour/hours (10鈧� per hour)\n"
            "- day/days (80鈧� per day)\n"
            "- week/weeks (500鈧� per week)"
        ), parse_mode='Markdown')
        return
    ...
    try:
        ...
        if not update_admin_balance(str(user_id), price):
            ...
            bot.send_message(chat_id, 
                ...
                f"Required: {price}鈧筡n"
                f"Your balance: {current_balance}鈧�", 
                parse_mode='Markdown')
            return
        ...
        success_msg = (
            ...
            f"Price: {price}鈧筡n"
            f"Remaining balance: {new_balance}鈧筡n\n"
        ...
        # Log the transaction
        logging.info(f"Admin {user_id} generated key worth {price}鈧� for {amount} {time_unit}")
...
@bot.message_handler(func=lambda message: message.text == "馃殌 饾悁饾惌饾惌饾悮饾悳饾悿")
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*饾悩饾惃饾惍 饾悮饾惈饾悶 饾惂饾惃饾惌 饾惈饾悶饾悹饾悽饾惉饾惌饾悶饾惈饾悶饾悵. 饾悘饾惀饾悶饾悮饾惉饾悶 饾惈饾悶饾悵饾悶饾悶饾惁 饾悁 饾悿饾悶饾惒 饾悡饾惃 饾悗饾惏饾惂饾悶饾惈:- 釟� @Nawab79 釟�*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*饾悩饾惃饾惍饾惈 饾悿饾悶饾惒 饾悺饾悮饾惉 饾悶饾惐饾惄饾悽饾惈饾悶饾悵. 饾悘饾惀饾悶饾悮饾惉饾悶 饾惈饾悶饾悵饾悶饾悶饾惁 饾悁 饾悿饾悶饾惒 饾悡饾惃 饾悗饾惏饾惂饾悶饾惈:- 釟� @Nawab79 釟�.*", parse_mode='Markdown')
        return
...
@bot.message_handler(func=lambda message: message.text == "饾悓饾惒 饾悁饾悳饾悳饾惃饾惍饾惂饾惌馃彟")
def my_account(message):
    ...
    if is_super_admin(user_id):
            account_info = (
                "馃憫---------------饾悁饾悵饾惁饾悽饾惂 饾悆饾悮饾惉饾悺饾悰饾惃饾悮饾惈饾悵---------------馃憫       \n\n"
                "馃専  饾棓饾棸饾棸饾椉饾槀饾椈饾榿 饾棗饾棽饾榿饾棶饾椂饾椆饾榾               \n"
                "隃贬礇岽�岽涐礈隃�: Super Admin\n"
                "岽�岽勧磩岽囮湵隃� 薀岽囜礌岽囀�: Unlimited\n"
                "岽樖�瑟岽犐熱磭散岽囮湵: Full System Control\n\n"
                "馃捈  饾棧饾棽饾椏饾椇饾椂饾榾饾榾饾椂饾椉饾椈饾榾 \n"
                "鈥� Generate Keys\n"
                "鈥� Manage Admins\n"
                "鈥� System Configuration\n"
                "鈥� Unlimited Balance"
            )
            ...
            balance = get_admin_balance(user_id)
            account_info = (
                "馃洝锔�---------------饾悁饾悵饾惁饾悽饾惂 饾悘饾惈饾惃饾悷饾悽饾惀饾悶---------------馃洝锔弉\n"
                f"馃挵  饾棔饾棶饾椆饾棶饾椈饾棸饾棽: {balance}鈧筡n\n"
                "馃寪  饾棓饾棸饾棸饾椉饾槀饾椈饾榿 饾棪饾榿饾棶饾榿饾槀饾榾:\n"
                "鈥� 蕗岽徥熱磭: Admin\n"
                "鈥� 岽�岽勧磩岽囮湵隃�: Restricted\n"
                "鈥� 岽樖�瑟岽犐熱磭散岽囮湵:\n"
                "  - Generate Keys\n"
                "  - User Management\n"
                "  - Balance Tracking"
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                "饾悩饾惃饾惍饾惈 饾悿饾悶饾惒 饾悺饾悮饾惉 饾悶饾惐饾惄饾悽饾惈饾悶饾悵. 饾悘饾惀饾悶饾悮饾惉饾悶 饾惈饾悶饾悵饾悶饾悶饾惁 饾悮 饾惂饾悶饾惏 饾悿饾悶饾惒.\n"
                "Contact 釟� @Nawab79 釟� for assistance."
            )
        else:
            account_info = (
                f"饾晲饾暊饾暒饾暎 饾敻饾晹饾晹饾暊饾暒饾暉饾暐 饾晙饾暉饾晽饾暊饾暎饾暈饾晵饾暐饾暁饾暊饾暉:\n\n"
                f"岽滉湵岽囀�纱岽�岽嶀磭: {found_user.get('username', 'N/A')}\n"
                f"岽犪磤薀瑟岽� 岽溕瘁礇瑟薀: {valid_until}\n"
                f"岽樖熱磤纱: {found_user.get('plan', 'N/A')}\n"
                f"岽勧礈蕗蕗岽嚿瘁礇 岽浬磵岽�: {current_time}"
            )
    else:
        account_info = "饾悘饾惀饾悶饾悮饾惉饾悶 饾惈饾悶饾悵饾悶饾悶饾惁 饾悁 饾悿饾悶饾惒 饾悡饾惃 饾悗饾惏饾惂饾悶饾惈:- 釟汙Nawab79 釟�."
